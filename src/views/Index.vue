<template>
  <el-container class="body-container">
    <el-aside width="174px">
      ...
      <el-menu :default-active="activePath" background-color="transparent" text-color="#dfdede" active-text-color="#01D4F9" router>
        <div v-for="(item, index) in navMenuList" :key="index">
          <el-submenu v-if="item.children" index="1">
            ...
            <el-menu-item v-for="(item1, index1) in item.children" :key="index1" :index="item1.path" @click="saveNavStatus(item1.path, item1.title)">
              <span slot="title">{{ item1.title }}</span>
            </el-menu-item>
          </el-submenu>
          <el-menu-item v-else :index="item.path" @click="saveNavStatus(item.path, item.title)">
            <i :class="item.icon"></i>
        ...
        <div class="main-left">
          ...
          <div class="lt">
            <div class="lt-title">
              ...
              <span class="date-label" style="margin-left: 70px; margin-right: 12px">学期</span>
              <el-date-picker class="date-picker" v-model="dateRangeValue" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" size="small" style="width: 200px; border-radius: 0"></el-date-picker>
              <span class="date-label" style="margin-left: 30px; margin-right: 12px">月份 </span>
              <el-date-picker class="date-picker" v-model="dateValue" type="month" placeholder="选择月份" size="small" style="width: 128px"> </el-date-picker>
            </div>
          ...
          <div class="lt lb">
            <div class="lt-title">
              ...
              <span class="date-label" style="margin-left: 70px; margin-right: 12px">学期</span>
              <el-date-picker class="date-picker" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" size="small" style="width: 200px; border-radius: 0"></el-date-picker>
              <span class="date-label" style="margin-left: 30px; margin-right: 12px">月份 </span>
              <el-date-picker class="date-picker" type="month" placeholder="选择月份" size="small" style="width: 128px"> </el-date-picker>
            </div>
            ...
            <div class="rl-main">
              <vuescroll :ops="ops">
                <div v-for="(item, index) in schoolList" :key="index">
                  <div :class="['list-item', index === 0 ? 'list-item1' : '', index === 1 ? 'list-item2' : '', index === 2 ? 'list-item3' : '']">
                    ...
                    <p style="width: 268px" :class="[index === 0 ? 'p-no1' : '', index === 1 ? 'p-no2' : '', index === 2 ? 'p-no3' : '']">
                      {{ item.name }}
                    </p>
                    <p :class="[index === 0 ? 'p-no1' : '', index === 1 ? 'p-no2' : '', index === 2 ? 'p-no3' : '']">52</p>
                  </div>
                </div>
              </vuescroll>
            </div>
...
.lt {
  width: 750px;
  height: 450px;
  background: linear-gradient(135deg, transparent 35px, rgba(0, 168, 255, 0.2) 0);
}
.lt-title {
  position: relative;
  display: flex;
...
.rr {
  ...
  width: 450px;
  height: 925px;
  background: linear-gradient(225deg, transparent 35px, rgba(0, 168, 255, 0.2) 0);
  margin-left: 24px;
}
